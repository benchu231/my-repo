...
import com.linbit.linstor.proto.javainternal.c2s.IntObsRemoteOuterClass.IntObsRemote;
...
import com.linbit.linstor.proto.javainternal.s2c.*;
...
public class ProtoCtrlStltSerializerBuilder extends ProtoCommonSerializerBuilder
    implements CtrlStltSerializerBuilder
{
    ...
    )
    {
        try
        {
            ...
            ArrayList<IntObsRemote> serializedObsRemotes = new ArrayList<>();
            ...
            for (AbsRemote remote : remotes)
            {
                ...
                {
                    serializedObsRemotes.add(remoteSerializerHelper.buildObsRemoteMsg((ObsRemote) remote));
                }
            ...
            MsgIntApplyFullSync.Builder builder = MsgIntApplyFullSync.newBuilder()
                ...
                .addAllObsRemotes(serializedObsRemotes)
    ...
    )
    {
        try
        {
            ...
            else if (remoteRef instanceof ObsRemote)
            {
                builder.setObsRemote(remoteSerializerHelper.buildObsRemoteMsg((ObsRemote) remoteRef));
            }
    ...
    )
    {
        try
        {
            MsgIntApplyRscSuccess.Builder builder = MsgIntApplyRscSuccess.newBuilder()
                .setRscId(
                    IntObjectId.newBuilder()
    ...
    @Override
    public CtrlStltSerializerBuilder notifyResourceFailed(Resource resource, ApiCallRc apiCallRc)
    {
        try
        {
    ...
    @Override
    public CtrlStltSerializerBuilder updateFreeCapacities(
    ...
    @Override
    public CtrlStltSerializerBuilder requestPhysicalDevices(boolean filter)
    {
        try
        {
    ...
    @Override
    public CtrlStltSerializerBuilder createDevicePool(
    ...
    @Override
    public CtrlStltSerializerBuilder deleteDevicePool(
        List<String> devicePaths,
        DeviceProviderKind providerKindRef,
        String poolName
    )
    ...
    @Override
    public CtrlStltSerializerBuilder physicalDevices(List<LsBlkEntry> entries)
    {
        try
        {
    ...
    private class RemoteSerializerHelper
    {
        ...
        private IntObsRemote buildObsRemoteMsg(ObsRemote obsRemote)
            throws AccessDeniedException
        {
            IntObsRemote.Builder builder = IntObsRemote.newBuilder()
                .setUuid(obsRemote.getUuid().toString())
                .setName(obsRemote.getName().displayValue)
                .setFlags(obsRemote.getFlags().getFlagsBits(serializerCtx))
                .setEndpoint(obsRemote.getUrl(serializerCtx))
                .setBucket(obsRemote.getBucket(serializerCtx))
                .setRegion(obsRemote.getRegion(serializerCtx))
                .setAccessKey(ByteString.copyFrom(obsRemote.getAccessKey(serializerCtx)))
                .setSecretKey(ByteString.copyFrom(obsRemote.getSecretKey(serializerCtx)));
            return builder
                .build();
        }
    ...
    public static StorPoolFreeSpace.Builder buildStorPoolFreeSpace(
        Entry<StorPool, Either<SpaceInfo, ApiRcException>> entry
    )
    {
        StorPool storPool = entry.getKey();
