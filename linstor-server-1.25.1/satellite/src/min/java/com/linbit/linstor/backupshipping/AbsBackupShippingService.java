...
public abstract class AbsBackupShippingService implements SystemService
{
    ...
    ) throws StorageException, InvalidNameException, InvalidKeyException, AccessDeniedException
    {
        if (RscLayerSuffixes.shouldSuffixBeShipped(rscNameSuffixRef))
        {
            ...
            String obsSuffix = stltConfigAccessor.getReadonlyProps()
                .getProp(ApiConsts.KEY_BACKUP_OBS_SUFFIX, ApiConsts.NAMESPC_BACKUP_SHIPPING);
            ...
            {
                backupName = new S3VolumeNameInfo(
                    rscNameRef,
                    rscNameSuffixRef,
                    vlmNrRef,
                    backupTime,
            ...
            {
                backupName = new ObsVolumeNameInfo(
                    rscNameRef,
                    rscNameSuffixRef,
                    vlmNrRef,
                    backupTime,
        ...
        if (serviceStarted)
        {
            if (!alreadyStarted(snapVlmData))
            {
                ...
                try
                {
                    ...
                    {
                        info.objMetaKey = new ObsMetafileNameInfo(
                    ...
                    {
                        info.objMetaKey = new S3MetafileNameInfo(
                            snap.getResourceName().displayValue,
                            backupTime,
                            s3Suffix,
                            snap.getSnapshotName().displayValue
                    ...
                    if (basedOnSnapVlmData != null && basedOnSnapVlmData != snapVlmData)
                    {
                        ...
                        if (remoteType.equals(RemoteType.OBS))
                        {
                            info.basedOnObjMetaKey = new ObsMetafileNameInfo(
                                basedOnSnap.getResourceName().displayValue,
                                basedOnBackupTime,
                                basedOnSnap.getSnapshotDefinition().getProps(accCtx)
                                    .getProp(ApiConsts.KEY_BACKUP_OBS_SUFFIX, ApiConsts.NAMESPC_BACKUP_SHIPPING),
                        ...
                        {
                            info.basedOnObjMetaKey = new S3MetafileNameInfo(
                                basedOnSnap.getResourceName().displayValue,
                                basedOnBackupTime,
                                basedOnSnap.getSnapshotDefinition().getProps(accCtx)
                                    .getProp(ApiConsts.KEY_BACKUP_S3_SUFFIX, ApiConsts.NAMESPC_BACKUP_SHIPPING),
        ...
        if (snapKey != null)
        {
            try (LockGuard lg = lockGuardFactory.build(LockType.READ, LockObj.RSC_DFN_MAP, LockObj.NODES_MAP))
            {
                synchronized (snap)
                {
                     ...
                     * s3/obs .meta file) so we should not even get into this then-case if a port was blocked
                     */
