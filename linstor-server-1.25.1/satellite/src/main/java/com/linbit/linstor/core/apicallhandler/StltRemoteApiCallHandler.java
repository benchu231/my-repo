...
import com.linbit.linstor.core.objects.ObsRemoteSatelliteFactory;
...
public class StltRemoteApiCallHandler
{
    ...
    private final ObsRemoteSatelliteFactory obsRemoteFactory;
    ...
    StltRemoteApiCallHandler(
        ...
        ObsRemoteSatelliteFactory obsRemoteFactoryRef,
    ...
    public void applyChangesObs(ObsRemotePojo obsremotePojo)
    {
        try
        {
            ObsRemote localObsRemote = obsRemoteFactory.getInstanceSatellite(
                apiCtx,
                obsremotePojo.getUuid(),
                LinstorParsingUtils.asRemoteName(obsremotePojo.getRemoteName()),
                obsremotePojo.getFlags(),
                obsremotePojo.getEndpoint(),
                obsremotePojo.getBucket(),
                obsremotePojo.getRegion(),
                obsremotePojo.getAccessKey(),
                obsremotePojo.getSecretKey()
            );

            localObsRemote.applyApiData(apiCtx, obsremotePojo);

            RemoteName remoteName = localObsRemote.getName();
            if (localObsRemote.getFlags().isSet(apiCtx, StltRemote.Flags.DELETE))
            {
                errorReporter.logTrace("ObsRemote marked for deletion. Deleting. %s", remoteName);
                remoteMap.remove(remoteName);
                localObsRemote.delete(apiCtx);
            }
            transMgrProvider.get().commit();
