...
public class StltApiCallHandler
{
    ...
    )
    {
        ...
        )
        {
            if (updateMonitor.getCurrentFullSyncId() == fullSyncId)
            {
                ...
                for (ObsRemotePojo obsRemote : obsRemotes)
                {
                    remoteHandler.applyChangesObs(obsRemote);
                }
                ...
                for (ObsRemotePojo obsRemote : obsRemotes)
                {
                    errorReporter.logTrace("Remote '" + obsRemote.getRemoteName() + "' received.");
                }
                for (EbsRemotePojo ebsRemote : ebsRemotes)
                {
    ...
    public void applyObsRemoteChanges(ObsRemotePojo obsRemotePojoRef)
    {
        applyChangedData(new ApplyObsRemote(obsRemotePojoRef));
    }
    ...
    private class ApplyObsRemote implements ApplyData
    {
        private final ObsRemotePojo obsRemotePojo;

        ApplyObsRemote(ObsRemotePojo obsRemotePojoRef)
        {
            obsRemotePojo = obsRemotePojoRef;
        }

        @Override
        public long getFullSyncId()
        {
            return obsRemotePojo.getFullSyncId();
        }

        @Override
        public long getUpdateId()
        {
            return obsRemotePojo.getUpdateId();
        }
        ...
        public void applyChange()
        {
            ...
            {
                remoteHandler.applyChangesObs(obsRemotePojo);
            }
        }
    }
