...
public class CtrlRemoteApiCallHandler
{
    ...
    public List<ObsRemotePojo> listObs()
    {
        final AccessContext pAccCtx = peerAccCtx.get();
        return listGeneric(
            ObsRemote.class,
    ...
    )
    {
        ...
        return scopeRunner.fluxInTransactionalScope(
            "Create obs remote",
            lockGuardFactory.buildDeferred(LockType.WRITE, LockObj.REMOTE_MAP),
            () -> createObsInTransaction(
                remoteName, endpointRef, bucketRef, regionRef, accessKeyRef, secretKeyRef, usePathStyleRef
            )
        ).transform(responses -> responseConverter.reportingExceptions(context, responses));
    }

    private Flux<ApiCallRc> createObsInTransaction(
        String remoteNameStr,
        String endpointRef,
        String bucketRef,
        String regionRef,
    ...
    public Flux<ApiCallRc> changeObs(
        String remoteName,
        String endpointRef,
        String bucketRef,
        String regionRef,
    ...
    private Flux<ApiCallRc> changeObsInTransaction(
        String remoteNameStr,
        String endpointRef,
        String bucketRef,
        String regionRef,
