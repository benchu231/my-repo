...
public class CtrlBackupApiHelper
{
    ...
    /**
     * Get all obs objects that can be verified to have been created by linstor
     */
    Map<String, ObsObjectInfo> loadAllLinstorObsObjects(
        ObsRemote obsRemoteRef,
        ApiCallRcImpl apiCallRc
    )
        throws AccessDeniedException
    {
        Set<String> allObsKeys = getAllObsKeys(obsRemoteRef, null);
        Map<String, ObsObjectInfo> ret = new TreeMap<>();
        // add all backups to the list that have usable metadata-files
        for (String obsKey : allObsKeys)
        {
            try
            {
                ObsMetafileNameInfo info = new ObsMetafileNameInfo(obsKey);
                // throws parse exception if not linstor json
                BackupMetaDataPojo obsMetaFile = obsBackupHandler.getMetaFile(
                    obsKey,
                    obsRemoteRef,
                    peerAccCtx.get(),
                    getLocalMasterKey()
                );
                ObsObjectInfo metaInfo = ret.computeIfAbsent(obsKey, ObsObjectInfo::new);
                metaInfo.exists = true;
                metaInfo.metaFile = obsMetaFile;
                for (List<BackupMetaInfoPojo> backupInfoPojoList : obsMetaFile.getBackups().values())
                {
                    for (BackupMetaInfoPojo backupInfoPojo : backupInfoPojoList)
                    {
                        String childObsKey = backupInfoPojo.getName();
                        ObsObjectInfo childObsObj = ret.computeIfAbsent(childObsKey, ObsObjectInfo::new);
                        childObsObj.referencedBy.add(metaInfo);
                        metaInfo.references.add(childObsObj);
                    }
                }
                ...
                if (snapDfn != null)
                {
                    if (snapDfn.getUuid().toString().equals(obsMetaFile.getSnapDfnUuid()))
                    {
                        metaInfo.snapDfn = snapDfn;
                    }
                    else
                ...
                String basedOnObsKey = obsMetaFile.getBasedOn();
                if (basedOnObsKey != null)
                {
                    ObsObjectInfo basedOnObsMetaInfo = ret.computeIfAbsent(basedOnObsKey, ObsObjectInfo::new);
                    basedOnObsMetaInfo.referencedBy.add(metaInfo);
                    metaInfo.references.add(basedOnObsMetaInfo);
                }
            }
            catch (MismatchedInputException exc)
            {
            ...
            {
                String errRepId = errorReporter.reportError(exc, peerAccCtx.get(), null, "used obs key: " + obsKey);
                apiCallRc.addEntry(
                    "IO exception while parsing metafile " + obsKey + ". Details in error report " + errRepId,
                    ApiConsts.FAIL_UNKNOWN_ERROR
                );
            }
            catch (ParseException ignored)
            ...
            try
            {
                ObsVolumeNameInfo info = new ObsVolumeNameInfo(obsKey);
                ObsObjectInfo obsDataInfo = ret.computeIfAbsent(obsKey, ObsObjectInfo::new);
                obsDataInfo.exists = true;
                obsDataInfo.snapDfn = loadSnapDfnIfExists(info.rscName, info.snapName);
            }
            catch (ParseException ignored)
            {
                // Ignored, not a volume file
