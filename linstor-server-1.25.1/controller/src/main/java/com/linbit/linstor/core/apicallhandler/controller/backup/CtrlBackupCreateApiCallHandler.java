...
public class CtrlBackupCreateApiCallHandler
{
    ...
    )
    {
        try {
            ...
            AbsRemote remote = remoteMap.get(new RemoteName(remoteNameRef));
            if (remote == null)
            {
                throw new ImplementationError(
                        "Remote " + remoteNameRef + " must not be null if the property is set"
                );
            }
    ...
    )
    {
        ...
        try
        {
            ...
            if (
                    remoteTypeRef.equals(RemoteType.S3) ||
                    remoteTypeRef.equals(RemoteType.OBS)
            )
            {
            ...
            {
                // only do this for s3/obs, l2l does it on its own later on
        ...
        throws AccessDeniedException, DatabaseException, InvalidKeyException, InvalidValueException
    {
        ...
        // save the s3/obs suffix as prop so that when restoring the satellite can reconstruct the .meta name
        // (s3/obs suffix is NOT part of snapshot name)
        String s3Suffix = sysCfgRepo.getStltConfForView(peerAccCtx.get()).getProp(
            ApiConsts.KEY_BACKUP_S3_SUFFIX,
            ApiConsts.NAMESPC_BACKUP_SHIPPING
        );
        String obsSuffix = sysCfgRepo.getStltConfForView(peerAccCtx.get()).getProp(
            ApiConsts.KEY_BACKUP_OBS_SUFFIX,
            ApiConsts.NAMESPC_BACKUP_SHIPPING
        );
        if (s3Suffix != null)
        {
    ...
    private Node chooseNode(
        ...
        String prefNode,
        ApiCallRcImpl responses,
        Map<ExtTools, Version> optionalExtToolsMap
    )
        throws AccessDeniedException
    {
