...
import com.linbit.linstor.backupshipping.AbsMetafileNameInfo;
import com.linbit.linstor.backupshipping.ObsMetafileNameInfo;
import com.linbit.linstor.backupshipping.S3MetafileNameInfo;
import com.linbit.linstor.core.BackupInfoManager;
import com.linbit.linstor.core.apicallhandler.ScopeRunner;
import com.linbit.linstor.core.apicallhandler.controller.*;
...
public class CtrlBackupRestoreApiCallHandler
{
    ...
    ) throws AccessDeniedException, InvalidNameException
    {
        ...
        AbsMetafileNameInfo toRestore = null;
        ...
        if (backupId != null && !backupId.isEmpty())
        {
            ...
            try
            {
                ...
                {
                    toRestore = new ObsMetafileNameInfo(metaName);
                    objects = obsBackupHandler
                            .listObjects(toRestore.rscName, (ObsRemote) remote, peerAccCtx.get(), targetMasterKey);
                    // do not use backupHelper.getAllObsKeys here to avoid two listObjects calls since objects is needed
                    // later
                    objKeys = objects.stream().map(object -> ((ObsObject) object).getObjectKey())
                            .collect(Collectors.toCollection(TreeSet::new));
                }
                else if (remote instanceof S3Remote)
                {
                    toRestore = new S3MetafileNameInfo(metaName);
                    objects = backupHandler
                            .listObjects(toRestore.rscName, (S3Remote) remote, peerAccCtx.get(), targetMasterKey);
                    // do not use backupHelper.getAllS3Keys here to avoid two listObjects calls since objects is needed
                    // later
                    objKeys = objects.stream().map(object -> ((S3ObjectSummary) object).getKey())
                            .collect(Collectors.toCollection(TreeSet::new));
                }
            }
        ...
        else
        {
            ...
            if(remote instanceof ObsRemote)
            {
                objects = obsBackupHandler.listObjects(srcRscName, (ObsRemote) remote, peerAccCtx.get(), targetMasterKey);
                // do not use backupHelper.getAllObsKeys here to avoid two listObjects calls since objects is needed later
                objKeys = objects.stream().map(object -> ((ObsObject) object).getObjectKey())
                        .collect(Collectors.toCollection(TreeSet::new));
                toRestore = backupHelper.getLatestBackup(objKeys, srcSnapName, remote.getType());
            }
            else if (remote instanceof S3Remote)
            {
                objects = backupHandler.listObjects(srcRscName, (S3Remote) remote, peerAccCtx.get(), targetMasterKey);
        ...
        // Sanity check, now we should have a metafile, and it should be contained in the objKeys
        if (toRestore == null || !objKeys.contains(toRestore.toString()))
        {
            throw new ApiRcException(
                ApiCallRcImpl.simpleEntry(
                    ApiConsts.FAIL_INVLD_BACKUP_CONFIG | ApiConsts.MASK_BACKUP,
        ...
        try
        {
            ...
            AbsMetafileNameInfo currentMetaFile = toRestore;
            Map<StorPoolApi, List<BackupInfoVlmPojo>> storPoolInfo = new HashMap<>();
            List<Pair<AbsMetafileNameInfo, BackupMetaDataPojo>> metadataChain = new ArrayList<>();
            ...
            do
            {
                ...
                if (remote instanceof ObsRemote)
                {
                    metadata = obsBackupHandler.getMetaFile(
                ...
                {
                    metadata = backupHandler.getMetaFile(
                        currentMetaFile.toString(),
                        (S3Remote) remote,
                        peerAccCtx.get(),
                        targetMasterKey
            ...
            boolean first = true;
            for (Pair<AbsMetafileNameInfo, BackupMetaDataPojo> meta : metadataChain)
            {
                Pair<Long, Long> totalSizes = new Pair<>(0L, 0L); // dlSize, allocSize
                backupApiCallHandler
                    .fillBackupInfo(first, storPoolInfo, objects, meta.objB, meta.objB.getLayerData(), totalSizes, remote.getType());
                first = false;
            }
            ...
            for (Pair<AbsMetafileNameInfo, BackupMetaDataPojo> metadata : metadataChain)
            {
                Snapshot snap = createSnapshotByObjMeta(
                    metadata.objA,
                    storPoolMap,
                    node,
                    targetRscName,
     ...
     */
    private Snapshot createSnapshotByObjMeta(
        AbsMetafileNameInfo metafileNameInfo,
        Map<String, String> storPoolMap,
        Node node,
        String targetRscName,
        String passphrase,
        ...
        throws AccessDeniedException, ImplementationError, DatabaseException
    {
        ...
        for (List<BackupMetaInfoPojo> backupList : metadata.getBackups().values())
        {
            ...
            {
                if (!objKeys.contains(backup.getName()))
                {
                    throw new ApiRcException(
                        ApiCallRcImpl.simpleEntry(
