...
public interface RemoteRepository extends ProtectedObject
{
    ...
    default ObsRemote getObs(AccessContext accCtx, RemoteName remoteName) throws AccessDeniedException
    {
        AbsRemote remote = get(accCtx, remoteName);
        if (remote != null && !(remote instanceof ObsRemote))
        {
            throw new ApiRcException(
                ApiCallRcImpl.simpleEntry(
                    ApiConsts.FAIL_INVLD_REMOTE_NAME,
                    "Given Remote was not of class ObsRemote but of " + remote.getClass().getCanonicalName()
                )
            );
        }
